...
```{r ink-free, cache = F}
sysfonts::font_add("Ink Free", regular = "C:\\Windows\\Fonts\\Inkfree.ttf")
...
```{r embed-quiz-aesthetics, echo=FALSE}
...
palmerpenguins::penguins %>% drop_na() %>% 
  ...
  theme_minimal() + 
  theme(text = element_text(family = "Ink Free", 
                            size = 18,
                            colour = "firebrick4")) +
...
Items black we'll cover here, in <span style="color: red;">red</span> we'll cover this afternoon. In <span style="color: blue;">blue</span> will have to wait for a future workshop.
...
movies %>% mutate(movie = fct_reorder(movie, gross)) %>% 
  slice_head(n=10) %>% #BREAK
  ggplot(aes(movie, gross)) + #BREAK
  geom_col(fill = "firebrick4") + #BREAK
  scale_y_continuous(breaks = scales::breaks_extended(8), 
                     labels = scales::label_dollar(scale = 1)) + #BREAK
...
`r chunk_reveal("box-office", break_type = "user")`
...
dslabs::gapminder %>% 
  filter(year == "2007") %>% 
  group_by(continent) %>% 
  summarise(life_exp_2007 = mean(life_expectancy)) %>% #BREAK
...
`r chunk_reveal("colplot-gapminder", break_type = "user")`
...
dslabs::gapminder %>% 
  ...
  coord_flip() +
  labs(caption = "@Data Gapminder (WHO)") #BREAK
```

`r chunk_reveal("dotplot-gapminder", break_type = "user")`
...
`r chunk_reveal("lollypop-gapminder", break_type = "user")`
...
dslabs::gapminder %>%
  ...
  mutate(continent = fct_reorder(continent, population)) %>% #BREAK
...
`r chunk_reveal("pie-gapminder", break_type = "user")`
